...
class AnalyzerTest {
  ...
  def testJoinAnalyzerSchemaWithValidation(): Unit = {
    val viewsGroupBy = getViewsGroupBy("join_analyzer_test.item_gb", Operation.AVERAGE) // snapshot accuracy, event source
    ...
    DataFrameGen
      .events(spark, itemQueries, 1000, partitions = 100) // event source
