...
class DerivationTest {
  ...
  def testBootstrapToDerivations(): Unit = {
    runTestBootstrapToDerivations("test_derivations")
  }

  @Test
  def testBootstrapToDerivationsWithExternalBootstrapSplit(): Unit = {
    spark.conf.set(SparkConstants.ChrononSplitExternalPartsBootstrap, "true")
    runTestBootstrapToDerivations("test_derivations_external_bootstrap_split")
  }
  ...
  private def runTestBootstrapToDerivations(namespace: String): Unit = {
    spark.sql(s"CREATE DATABASE IF NOT EXISTS $namespace")
    val groupBy = BootstrapUtils.buildGroupBy(namespace, spark)
