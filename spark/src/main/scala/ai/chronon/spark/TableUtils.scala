...
trait BaseTableUtils {
  ...
  def insertPartitions(df: DataFrame,
    ...
    repartitionAndWrite(finalizedDf, tableName, saveMode, stats = stats, sortByCols = sortByCols, tableProperties = tableProperties,  allowEmpty = allowEmpty)
  }
  ...
  def insertUnPartitioned(df: DataFrame,
    ...
    repartitionAndWrite(df, tableName, saveMode, stats = None, tableProperties = tableProperties, partition = false)
  }
  ...
  private def repartitionAndWrite(df: DataFrame,
    ...
    logger.info(s"starting repartitionAndWrite...")
    wrapWithCache(s"repartition & write to $tableName", df) {
      logger.info(s"Repartitioning before writing...")
      repartitionAndWriteInternal(df, tableName, saveMode, stats, sortByCols, partition, tableProperties, allowEmpty)
    }.get
  }
  ...
  private def repartitionAndWriteInternal(df: DataFrame,
    ...
    if (allowEmpty && emptyResults) {
      ...
      val (dailyFileCount, tablePartitionCount) = getDailyFileCountAndTablePartitionCount(df, tableName, stats)
  ...
  private def getDailyFileCountAndTablePartitionCount(df: DataFrame, tableName: String, stats: Option[DfStats]): (Int, Int) = {
    ...
    outputParallelism match {
      case Some(dailyFileCount) =>
        ...
        logger.info(s"outputParallelism is defined. dailyFileCount = ${dailyFileCount}, tablePartitionCount = ${tablePartitionCount}")
