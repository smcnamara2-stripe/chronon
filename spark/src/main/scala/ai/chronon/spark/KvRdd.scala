...
case class TimedKvRdd(data: RDD[(Array[Any], Array[Any], Long)],
  ...
  def toAvroDf: DataFrame = {
    val jsonSamplingPercent = sparkSession.conf.get(SparkConstants.ChrononJsonSamplingPercent, "1").toDouble
    val avroRdd: RDD[Row] = data.map {
      case (keys, values, ts) =>
        val (keyJson, valueJson) = if (math.random < jsonSamplingPercent / 100) {
          (keyToJson(keys), valueToJson(values))
        } else {
          (null, null)
        }
