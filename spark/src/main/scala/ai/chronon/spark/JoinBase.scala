...
abstract class JoinBase(joinConf: api.Join,
          ...
          unfilledRangeCombined
            .foreach(unfilledRange => {
              ...
              if (filledDf.isDefined) {
                ...
                filledDf.get.saveWithTableUtils(
                                  tableUtils,
                                  partTable,
                                  tableProps,
  ...
  def computeJoinPart(leftDfWithStats: Option[DfWithStats],
    ...
    val rightDf = (joinConf.left.dataModel, joinPart.groupBy.dataModel, joinPart.groupBy.inferredAccuracy) match {
      case (Entities, Events, _)   => partitionRangeGroupBy.snapshotEvents(unfilledRange, snapshotResolution)
  ...
  def computeJoin(stepDays: Option[Int] = None,
                  ...
                  runValidations: Boolean = false): DataFrame = {
    computeJoinOpt(stepDays, overrideStartPartition, runValidations).get
  }

  def computeJoinOpt(stepDays: Option[Int] = None, overrideStartPartition: Option[String] = None, runValidations: Boolean = false): Option[DataFrame] = {
    ...
    val runSmallMode = {
      if (tableUtils.smallModelEnabled) {
        val thresholdCount =
          leftDf(joinConf, wholeRange, tableUtils, limit = Some(tableUtils.smallModeNumRowsCutoff + 1), allowEmpty = allowEmpty).get.count()
    ...
    effectiveRanges.zipWithIndex.foreach {
      case (range, index) =>
        ...
        leftDf(joinConf, range, tableUtils, allowEmpty = allowEmpty).map { leftDfInRange =>
          ...
          if (selectedJoinParts.isDefined) {
            ...
            finalDf.get.saveWithTableUtils(tableUtils, outputTable, tableProps, autoExpand = true, allowEmpty = allowEmpty)
