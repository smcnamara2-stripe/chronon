...
class LogFlattenerJob(session: SparkSession,
  ...
  private def fetchSchemas(hashes: Seq[String]): Map[String, String] = {
    ...
    logger.info(s"fetchSchemas() get lastAvailablePartition of schemaTable: ${schemaTableDs.get}.")
  ...
  def buildLogTable(): Unit = {
    ...
    val rowCounts = unfilledRanges.map { unfilled =>
      ...
      logger.info("schemaHashes in unfilledRanges: " + schemaHashes.mkString(","))
...
object LogFlattenerJob {
  ...
  def readSchemaTableProperties(tableUtils: BaseTableUtils, logTable: String): Map[String, String] = {
