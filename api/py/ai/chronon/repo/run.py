...
def check_call(cmd):
    ...
    return subprocess.check_call(cmd.split(), bufsize=0) # nosemgrep no user-supplied input


def check_output(cmd):
    print("Running command: " + cmd)
    return subprocess.check_output(cmd.split(), bufsize=0).strip() # nosemgrep no user-supplied input
