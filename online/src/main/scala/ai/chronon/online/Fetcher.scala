...
class Fetcher(val kvStore: KVStore,
  ...
  private def logResponse(resp: ResponseWithContext, ts: Long): Response = {
    ...
    val loggingTry: Try[Unit] = joinCodecTry.map(codec => {
      // Adding additional logs on JoinCodec & request keys
      if (debug || flagStore.isSet("zoolander.shepherd.enable_async_feature_log_debug", Map("join_name" -> codec.conf.join.join.metaData.name).asJava)) {
        println(
          s"""In logResponse, before encoding keys:
             |JoinCodec keySchema name: ${codec.keySchema.name}
             |JoinCodec keySchema fields: ${codec.keySchema.fields.mkString("Array(", ", ", ")")}
             |JoinCodec key AvroCodec schema: ${codec.keyCodec.schemaStr}
             |JoinCodec key AvroCodec fieldNames: ${codec.keyCodec.fieldNames.mkString("Array(", ", ", ")")}
             |Request keys: ${resp.request.keys}""".stripMargin)
      }
